document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadCities() {
        ...
        const heuristicsScript = document.getElementById('heuristics-data');
        if (heuristicsScript) {
            currentHeuristics = JSON.parse(heuristicsScript.textContent);
        }
        ...
        for (const city in currentCities) {
            ...
            cityItem.innerHTML = `
                <div>
                    ...
                    <div class="connections">${connections} koneksi</div>
                </div>
                <div>✏️</div>
            `;
    ...
    function drawGraphPreview() {
        ...
        for (const from in currentCities) {
            for (const to in currentCities[from]) {
                ...
                const midX = (posFrom[0] + posTo[0]) / 2;
                const midY = (posFrom[1] + posTo[1]) / 2;
    ...
    // Event binding
    addConnectionBtn.addEventListener('click', () => addConnectionField());
    addCityBtn.addEventListener('click', openAddForm);
